...
func (u *UserHandler) GenerateAccessToken(w http.ResponseWriter, r *http.Request) {
	var dto dto.RefreshAccessTokenRequest
	if err := json.NewDecoder(r.Body).Decode(&dto); err != nil {
		handlersUtils.HandleError(
			exceptions.NewBadRequestSimple("Invalid JSON", err.Error()), w)
		return
	}
	result, err := u.Service.GenerateAccessTokenFromRefreshToken(&dto)
	if err != nil {
		handlersUtils.HandleError(err, w)
		return
	}
...
func (u *UserHandler) Authorizate(w http.ResponseWriter, r *http.Request) {
	var dto dto.AuthorizationRequest
	if err := json.NewDecoder(r.Body).Decode(&dto); err != nil {
		handlersUtils.HandleError(
			exceptions.NewBadRequestSimple("Invalid JSON", err.Error()), w)
		return
	}
	result, err := u.Service.Authorizate(r.Context(), &dto)
	if err != nil {
		handlersUtils.HandleError(err, w)
		return
	}
...
func (u *UserHandler) Logout(w http.ResponseWriter, r *http.Request) {
	var dto dto.LogoutRequest
	if err := json.NewDecoder(r.Body).Decode(&dto); err != nil {
		handlersUtils.HandleError(
			exceptions.NewBadRequestSimple("Invalid JSON", err.Error()), w)
		return
	}
	err := u.Service.Logout(r.Context(), &dto)
	if err != nil {
		handlersUtils.HandleError(err, w)
		return
	}
